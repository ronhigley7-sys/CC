...
(function(){
  ...
  function unavailable(dateKey,fsmShift,name){
    return fsmHistory().some(e=>e&&e.date===dateKey&&e.staff===name&&e.shift===fsmShift&&(e.assign==='Float'||e.assign==='Sitter'));
  }

  function sortRot(names,hist,prevFirst,chargeCheck){
  ...
  function buildCA(dateKey,shiftKey){
    const store=loadCA();store[dateKey]=store[dateKey]||{};const all=eligibleCA(dateKey,shiftKey),saved=store[dateKey][shiftKey];
    if(Array.isArray(saved)&&saved.length===all.length&&saved.every(n=>all.includes(n))&&all.every(n=>saved.includes(n)))return saved;
    const hist=caHist(shiftKey,store),prior=((store[prevDate(dateKey)]||{})[shiftKey])||[],prev=prior[0]||'';const ag=all.filter(isAgency),reg=all.filter(n=>!isAgency(n));let out=[];
    if(ag.length){const indep=ag.filter(n=>!isOrientee(n)),pool=indep.length?indep:ag,lead=sortRot(pool,hist,prev)[0];if(lead)out.push(lead);out.push(...sortRot(ag.filter(n=>n!==lead&&!isOrientee(n)),hist,''));out.push(...sortRot(ag.filter(n=>n!==lead&&isOrientee(n)),hist,''));}
    out.push(...sortRot(reg.filter(n=>!isOrientee(n)),hist,ag.length?'':prev));out.push(...sortRot(reg.filter(isOrientee),hist,''));out=uniq(out);all.forEach(n=>{if(!out.includes(n))out.push(n);});store[dateKey][shiftKey]=out;saveCA(store);return out;
  }
